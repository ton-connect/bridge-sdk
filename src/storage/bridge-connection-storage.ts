...
import { BridgeSessionRaw } from '../models/bridge/bridge-session-raw';
import {
    BridgeConnection,
...
} from '../models/bridge/bridge-connection';
...
export class BridgeConnectionStorage {
    ...
    public async getHttpConnection(): Promise<BridgeConnectionHttp | BridgePendingConnectionHttp> {
        ...
        if (!connection) {
            throw new BridgeSdkError('Trying to read HTTP connection source while nothing is stored');
        }

        if (connection.type === 'injected') {
            throw new BridgeSdkError('Trying to read HTTP connection source while injected connection is stored');
        }
    ...
    public async getHttpPendingConnection(): Promise<BridgePendingConnectionHttp> {
        ...
        if (!isPendingConnectionHttp(connection)) {
            throw new BridgeSdkError('Trying to read HTTP-pending connection while http connection is stored');
        }
    ...
    public async getInjectedConnection(): Promise<BridgeConnectionInjected> {
        ...
        if (!connection) {
            throw new BridgeSdkError('Trying to read Injected bridge connection source while nothing is stored');
        }
